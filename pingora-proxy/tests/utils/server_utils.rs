...
impl ProxyHttp for ExampleProxyHttps {
    ...
    async fn upstream_peer(
        ...
        let alt = req.headers.get("alt").map(|v| v.to_str().unwrap().to_string());
        ...
        if let Some(psk_identity) = req.headers.get("psk_identity") {
            use pingora_core::{protocols::tls::{Psk, PskConfig}, tls::PskHmac};

            let psk = Psk::new(psk_identity.to_str().unwrap().to_string(), TEST_PSK_SECRET.as_bytes().to_vec(), PskHmac::SHA256);
...
impl PskTlsServer {
    ...
    async fn run_server() {
        use pingora_core::{protocols::tls::{Psk, PskConfig, PskType}, tls::{Config, PskHmac, S2NPolicy, DEFAULT_TLS13}};
        use tokio::net::TcpListener;
        use pingora_core::{protocols::tls::S2NConnectionBuilder, tls::TlsAcceptor};

        let psk = Psk::new(TEST_PSK_IDENTITY.to_string(), TEST_PSK_SECRET.as_bytes().to_vec(), PskHmac::SHA256);
        ...
        loop {
            use pingora_core::protocols::tls::AutoFlushableStream;
